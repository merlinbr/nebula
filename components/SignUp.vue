  ...
  <div class="grid grid-cols-1">
    <label class="block mb-4">
      ...
      <input
        ...
        type="text"
        ref="username"
        class="mt-0 w-full block px-0.5 border-b border-turquoise-500 text-white focus:outline-none border-current transparent bg-dark"
      >
    </label>
    <button
      @click="createAccount"
    ...
    <h2 class="hz-line-text">
      <span>OR</span>
    </h2>
    <button
      @click="$emit('toggle-login', true)"
...
<script>
  export default {
    ...
    methods: {
      ...
      createAccount() {
        this.$axios.post(`https://api.spacetraders.io/users/${this.username}/claim`).then(response => {
